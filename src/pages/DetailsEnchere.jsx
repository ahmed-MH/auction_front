...
const ProductDetails = () => {
  ...
  const handleBid = async () => {
    ...
    if (currentUser.soldeCredit < bidAmount) {
      ...
      if (window.confirm(`Solde insuffisant (Manque ${manquant.toFixed(2)} DT). Voulez-vous recharger votre compte ?`)) {
        navigate("/account");
      }
      return;
    }
  ...
  const handleWishlist = () => {
    ...
    if (!exists) {
      ...
      alert("❤️ Produit ajouté à votre liste de souhaits !");
    } else {
      alert("⚠️ Ce produit est déjà dans votre liste.");
    }
  };
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              ...
              {isOwner && !isEnded && (
                <div className="flex gap-4 w-full">
                   ...
                   <button onClick={async () => { if(window.confirm("Supprimer ?")) { await api.delete(`/api/encheres/${id}`); navigate("/products"); }}} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-red-100 text-red-700 rounded-xl font-bold hover:bg-red-200 transition"><Trash2 className="w-4 h-4"/> Supprimer</button>
                </div>
              )}
            </div>
